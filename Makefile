...
storage-up: teardown-docker 
	@(docker container inspect clh > /dev/null && docker start clh) || (docker run -d -p 9000:9000 --network local-net -e CLICKHOUSE_USER=user -e CLICKHOUSE_PASSWORD=pwd -e CLICKHOUSE_DEFAULT_ACCESS_MANAGEMENT=1 --name clh --ulimit nofile=262144:262144 clickhouse/clickhouse-server:25.3)
